...
// How many possible txid's are there?  16 ^ 64
...
// Convert this URL-safe string into a JS object representing the target
...
exports.Account = function(rpcConfig, address, privateKey, timeout, minConf) {
  ...
  // The txid of the previous tx and the vout index that paid money into my
  // address
  //
  // TODO: store multiple of these
  var previousTxid, previousVout;
  
  // How much money is in my address?
  var addressBalance;
  ...
  this.initialize = function(cb) {
    var initialize = function(queueCb) {
        ...
        function(err, transactions) {
          ...
          for (var i in transactions) {
            ...
            if (typeof addressBalance == 'undefined' ||
                transactionAmount > addressBalance) {
              previousTxid = transactions[i].txid;
              previousVout = transactions[i].vout;
              addressBalance = transactionAmount;
            }
          }

          if(addressBalance < AMOUNT) {
            queueCb("You don't have enoungh Bitcoins.  You need at least "
                    + AMOUNT + ' Satoshis to use this.');
          } else if (!addressBalance) {
            queueCb(
    ...
    var requestVoucher = function(queueCb) {
      ...
      async.waterfall([
        ...
        function(waterfallCb) {
          ...
          if (addressBalance > AMOUNT) {
            outputs[address] = (addressBalance - AMOUNT) / SATOSHIS;
            assert.equal(
          ...
          client.cmd(
            ...
            [{'txid': previousTxid, 'vout': previousVout}],
            outputs,
            waterfallCb
          );
        },
  ...
  this.createVoucher = function(destinationAddress, targetSerialized, cb) {
    ...
    var createVoucher = function(queueCb) {
      ...
      if(AMOUNT > addressBalance) {
        queueCb('Need ' + AMOUNT + ', only have ' + addressBalance);
        return;
      }
      ...
      if (addressBalance > AMOUNT) {
        outputs[address] = (addressBalance - AMOUNT) / SATOSHIS;
        assert.equal(Math.round((outputs[destinationAddress] + outputs[address]) * SATOSHIS), addressBalance + AMOUNT);
      }
      ...
      client.cmd('createrawtransaction', [{'txid': previousTxidGuess, 'vout': target.vout}, {'txid': previousTxid, 'vout': previousVout}], outputs, function(err, transactionHex) {
        if (err) {
          queueCb('Error creating transaction: ' + err);
          return;
        }
        ...
        client.cmd('signrawtransaction', transactionHex, [{'txid': previousTxidGuess,'vout': target.vout, 'scriptPubKey': scriptPubKey, 'redeemScript': privateKey}], function(err, signed) {
          if (err) {
            queueCb('Error signing transaction: ' + err);
            return;
          }

          // Why is this transaction complete?  Whoooo knows.
